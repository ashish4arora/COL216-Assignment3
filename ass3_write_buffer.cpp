...
int main(int argc, char *argv[]) {
    ...
    while (trace >> op >> hex >> addr) {
        ...
        if (l1_hit) {
            ...
            queue<unsigned long> writebuffer; //will contain only 1 entry at max
            if (evicted && dirty) { //add to write buffer if there was a dirty block in that location already
                writebuffer.push(oldAddr);
            }
            ...
            // now if the write buffer contains any block, we write it
            if (writebuffer.size()) {
                l1_writebacks++;
                // do we add time for this?
                // get the address for which write back is due
                unsigned long oldAddr = writebuffer.front();
                writebuffer.pop();
